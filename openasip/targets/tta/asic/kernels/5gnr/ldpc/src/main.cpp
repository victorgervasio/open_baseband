	...
	for (unsigned i = 0; i < N0.size(); i++) {
		...
		while ((n_blks_done < N_MAX_BLKS) && (n_blk_errs[i] < 100)) {
			...
			if (n_blks_done % 10 == 0 || n_blks_done == 1) {
				//print_len = s//printf(str, "Elapsed time: %.1f seconds, # tx blocks: %d,# error blocks:%d, ber: %.5f, bler %.5f", float(clock() - tStart) / CLOCKS_PER_SEC, n_blks_done, n_blk_errs[i], ber[i], bler[i]);
		...
		//s//printf(str, "Elapsed time: %.1f seconds, # tx blocks: %d,# error blocks:%d, ber: %.5f, bler %.5f", static_cast<float>(clock() - tStart) / CLOCKS_PER_SEC, n_blks_done, n_blk_errs[i], ber[i], bler[i]);
		//cout << std::string(print_len, '\b') << str << flush;
